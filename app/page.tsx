...
import { localFilter, pickRandomDifferent } from "@/lib/randomize";
...
export default function Home() {
  ...
  const resolveCandidates = useCallback(
    async <T extends { name: string; tags: string[]; cuisine?: string }>(
      modeArg: Mode,
      list: readonly T[],
      currentHint: string,
    ): Promise<{ pool: readonly T[]; appliedHint: string | null; notice?: string }> => {
      const trimmed = currentHint.trim();
      if (!trimmed) return { pool: list, appliedHint: null };
      ...
      if (topN.length > 0) return { pool: topN, appliedHint: trimmed };

      const local = localFilter(list, trimmed).slice(0, 5);
      if (local.length > 0) return { pool: local, appliedHint: trimmed };
  ...
  const runEatAnimation = useCallback(
    async (modeArg: Mode) => {
      ...
      const { pool, appliedHint, notice: nMsg } = await resolveCandidates(
        modeArg,
        list,
        hint,
      );
      const chosen = pickRandomDifferent(pool, eatResult?.item ?? null);
  ...
  const runCookSingleAnimation = useCallback(
    async (meal: Meal) => {
      ...
      const chosen = pickRandomDifferent(pool, cook[meal]?.item ?? null);
      const t = window.setTimeout(() => {
  ...
  const runCookAllAnimation = useCallback(async () => {
    ...
    const picks = resolved.map((r) => ({
      meal: r.meal,
      item: pickRandomDifferent(r.pool, cook[r.meal]?.item ?? null) as CookItem,
  ...
  const cookShuffleItems = useMemo(
    () => ({
      breakfast: cookOptions.breakfast.map((c) => ({ name: c.name })),
      lunch: cookOptions.lunch.map((c) => ({ name: c.name })),
      dinner: cookOptions.dinner.map((c) => ({ name: c.name })),
    }),
    [],
  );
    ...
    <main className="mx-auto flex w-full max-w-4xl flex-1 flex-col gap-6 px-4 py-8 sm:gap-8 sm:px-6 sm:py-12">
      <header className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex h-9 w-9 items-center justify-center rounded-full border border-black/10 text-sm hover:bg-black/5 dark:border-white/10 dark:hover:bg-white/10"
      ...
      {mode !== "cook" ? (
        <section className="flex flex-col gap-4">
          <ResultCard
            result={
              eatResult
                ? { name: eatResult.item.name, subtitle: eatResult.item.cuisine }
                : { name: "Press Randomize", subtitle: "Press space or click below" }
            }
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            {(["breakfast", "lunch", "dinner"] as Meal[]).map((meal) => (
              <ResultCard
                ...
                size="md"
                result={
                  cook[meal] ? { name: cook[meal]!.item.name } : { name: "—" }
                }
      ...
      <section className="flex flex-col gap-3">
        ...
        <button
          ...
          className="self-start text-sm text-black/60 underline-offset-4 hover:text-black hover:underline dark:text-white/60 dark:hover:text-white"
        >
      ...
      {notice && (
        <div
          role="status"
          className="fixed inset-x-0 bottom-6 z-50 mx-auto w-fit max-w-[90%] rounded-full bg-black px-4 py-2 text-sm text-white shadow-lg dark:bg-white dark:text-black"
        >
          {notice.text}
        </div>
      )}
      ...
      <footer className="mt-auto pt-6 text-center text-xs text-black/40 dark:text-white/40">
        Tip: press{" "}
        <kbd className="rounded border border-black/10 px-1 dark:border-white/20">
          space
        </kbd>{" "}
        to randomize
      </footer>
